...
std::string CreateTemporaryTerminalInstallerScript(const std::string& installerCopyPath, std::string* errorMessage)
{
    ...
    contents += "echo\n";
    contents += "TARGET_USER=\"$(id -un)\"\n";
    contents += "USER_GROUPS=\" $(id -nG) \"\n";
    contents += "if command -v sudo >/dev/null 2>&1 && [[ \"$USER_GROUPS\" == *\" sudo \"* || \"$USER_GROUPS\" == *\" wheel \"* || \"$USER_GROUPS\" == *\" admin \"* ]]; then\n";
    contents += "  sudo env SMU_TARGET_USER=\"$TARGET_USER\" /bin/bash \"$INSTALLER_COPY\"\n";
    contents += "  status=$?\n";
    contents += "elif command -v su >/dev/null 2>&1; then\n";
    contents += "  echo \"This account is not configured for sudo; using the system administrator password instead.\"\n";
    contents += "  su -c \"SMU_TARGET_USER=$(printf '%q' \"$TARGET_USER\") /bin/bash $(printf '%q' \"$INSTALLER_COPY\")\" root\n";
    contents += "  status=$?\n";
    contents += "else\n";
    contents += "  echo \"Neither sudo nor su is available to run the installer as administrator.\" >&2\n";
    contents += "  status=1\n";
    contents += "fi\n";
    contents += "echo\n";
