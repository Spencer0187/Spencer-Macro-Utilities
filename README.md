...
| --- | --- | --- |
| Windows 10/11 x64 | `Spencer-Macro-Utilities-VX.X.X-Windows.exe` or `Spencer-Macro-Utilities-Windows.zip` | Prefer the correctly named direct executable. **V3.4.0 only:** the ZIP temporarily contains root-level `suspend.exe` so V3.0-V3.3.x auto-updaters can migrate; launching it once renames/relaunches it as `Spencer-Macro-Utilities-V3.4.0-Windows.exe` before the GUI appears. V3.4.1+ ZIPs use the clean `Spencer-Macro-Utilities/` + versioned-EXE layout. If Windows blocks a download, open Properties and select **Unblock**. |
| macOS 11+ | `xyz-macOS-universal.dmg` or `.zip` | Drag `Spencer Macro Utilities.app` to Applications, eject the DMG, then approve the first launch in **System Settings → Privacy & Security**. |
...
3. On Wayland, foreground-app detection remains restricted. The first Lua pixel read opens an in-app confirmation; accepting it opens the desktop portal's monitor picker and pauses the script until its first frame is ready. `moveMouseAbs()` separately asks for RemoteDesktop pointer-control permission and targets that session's selected monitor. The portal never reveals the current global cursor position, so relative operations that require it remain unavailable. If the portal is unavailable or the user declines, SMU returns a script error instead of attempting an X11 root-window read.
