...
static std::vector<DWORD> GetProcessIdByName(bool takeallprocessids)
{
    ...
    std::wstring targetNameW = stringToWstring(targetName);
    ...
    if (takeallprocessids)
    {
        ...
        if (Process32First(hSnapshot, &pe))
        {
            do
            {
                if (_wcsicmp((const wchar_t *)pe.szExeFile, targetNameW.c_str()) == 0)
                {
...
static std::string GetStringFromUrl(const wchar_t* url)
{
    ...
    HINTERNET hInternet = InternetOpenW(L"Spencer-Macro-Utilities-Updater", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
    if (!hInternet) return "";

    InternetSetOption(hInternet, INTERNET_OPTION_CONNECT_TIMEOUT, &timeout, sizeof(timeout));
    InternetSetOption(hInternet, INTERNET_OPTION_RECEIVE_TIMEOUT, &timeout, sizeof(timeout));

    HINTERNET hConnect = InternetOpenUrlW(hInternet, url, NULL, 0, INTERNET_FLAG_RELOAD | INTERNET_FLAG_NO_CACHE_WRITE, 0);
    if (!hConnect) {
...
void PerformUpdate(const std::string& newVersion, const std::string& localVersion) {
    ...
    if (urlTemplateAnsi.empty()) {
        MessageBoxW(NULL, L"Failed to retrieve update URL configuration.", L"Update Error", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    std::wstring downloadUrl = stringToWstring(urlTemplateAnsi);

    // 2. Download the zip file into a memory buffer
    std::vector<char> zipData;
    if (!DownloadToMemory(downloadUrl, zipData)) {
        MessageBoxW(NULL, L"Failed to download the update. Please check your internet connection.", L"Update Error", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    if (!ExtractFileFromMemory(zipData, exeNameInZip, exeData)) {
        MessageBoxW(NULL, L"Failed to extract the update from the downloaded package. It may be corrupt.", L"Update Error", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        MessageBoxW(NULL, L"Could not create temporary update file. Please check folder permissions.", L"Update Error", MB_OK | MB_ICONERROR);
        return;
    }
    DWORD bytesWritten;
    if (!WriteFile(hFile, exeData.data(), exeData.size(), &bytesWritten, NULL) || bytesWritten != exeData.size()) {
        ...
        MessageBoxW(NULL, L"Failed to write update data to temporary file.", L"Update Error", MB_OK | MB_ICONERROR);
        return;
    }
    CloseHandle(hFile);
    ...
    std::wstring wLocalVersion = stringToWstring(localVersion);
    std::wstring wNewVersion = stringToWstring(newVersion);
    ...
    if (hBatchFile != INVALID_HANDLE_VALUE) {
        std::string mbBatchScript = WstringToString(batchScriptContent);
        ...
        MessageBoxW(NULL, L"Could not create the updater script. Please check permissions.", L"Update Error", MB_OK | MB_ICONERROR);
        DeleteFileW(tempExePath.c_str());
    }
}
...
static void RunGUI()
{
	...
	ImGui::GetIO().Fonts->AddFontFromMemoryTTF(pData, size, 20.0f);
		...
		if (currentTime >= nextFrameTime) {
			...
			if (ImGui::Checkbox("##CamfixToggle", &camfixtoggle) || PreviousSensValue == -1) {
				...
				try {
					if (wallhopupdate) {
						...
						if (wallhopswitch) {
							wallhop_dx = std::lround(std::stoi(WallhopPixels) * (camfixtoggle ? -factor : factor));
							wallhop_dy = std::lround(std::stoi(WallhopPixels) * (camfixtoggle ? factor : -factor));
						} else {
							wallhop_dx = std::lround(std::stoi(WallhopPixels) * factor);
							wallhop_dy = std::lround(std::stoi(WallhopPixels) * -factor);
				...
				float CurrentWallWalkValue = std::stof(RobloxSensValue); // Wallwalk
				...
				float CurrentSensValue = std::stof(RobloxSensValue); // Speedglitch
			...
			if (selected_section >= 0 && selected_section < sections.size()) {
				...
				if (selected_section == 3) { // Speedglitch
					...
					float CurrentSensValue = std::stof(RobloxSensValue);
				...
				if (selected_section == 6) { // Wallhop
					...
					if (ImGui::InputText("##WallhopPixels", WallhopPixels, sizeof(WallhopPixels), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							wallhop_dx = std::lround(std::stoi(WallhopPixels));
							wallhop_dy = -std::lround(std::stoi(WallhopPixels));
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
					...
					if (ImGui::InputText("###", WallhopDelayChar, sizeof(WallhopDelayChar), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							WallhopDelay = std::lround(std::stoi(WallhopDelayChar));
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
				...
				if (selected_section == 10) { // Wall-Walk
					...
					float CurrentWallWalkValue = std::stof(RobloxSensValue);
					...
					try {
						RobloxWallWalkValueDelay = atoi(RobloxWallWalkValueDelayChar);
					} catch (const std::invalid_argument &e) {
					} catch (const std::out_of_range &e) {
					}
				...
				if (selected_section == 13) { // Bunnyhop
					...
					if (ImGui::InputText("##BunnyhopDelay", BunnyHopDelayChar, sizeof(BunnyHopDelayChar), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
							BunnyHopDelay = atoi(BunnyHopDelayChar);
						} catch (const std::invalid_argument &e) {
						} catch (const std::out_of_range &e) {
						}
					}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if (!remoteVersion.empty()) 
    {
        ...
        {
            std::wstring wRemoteVersion = stringToWstring(remoteVersion);
            std::wstring wLocalVersion = stringToWstring(localVersion);
	...
	bool tabbedintoroblox = IsForegroundWindowProcess(hProcess);
    {
		...
		if ((GetAsyncKeyState(vk_bouncekey) & 0x8000) && tabbedintoroblox && macrotoggled && notbinding && section_toggles[12]) {
			if (!isbounce) {
				int turn90 = std::lround((camfixtoggle ? 250 : 180) / atof(RobloxSensValue));
				int skey = 0x1F; // S key
				int dkey = 0x20; // D key
				int wkey = 0x11; // W key
	...
	//ImGui::DestroyContext(); //causes som error when closes am too tired to actually solve it kthx
	CleanupDeviceD3D();
	DestroyWindow(hwnd);
	timeEndPeriod(1);
	exit(0);
